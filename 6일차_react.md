...
> ⚠️ react는 최상위에 **Hook**을 호출하지 않는 경우, 잘못된 결과를 초래합니다.  
> 왜냐하면 특정 `state`가 **Hook**이 호출되는 순서에 의존하기 때문입니다.
>
> ```jsx
...
> useState('Poppins')        // 🔴 2 (원래는 3이었던). surname state 변수를 읽는 데 실패했습니다.
> useEffect(updateTitle)     // 🔴 3 (원래는 4였던). 제목을 업데이트하기 위한 effect가 대체되는 데 실패했습니다.
> ```
>
...
**의존성 배열**을 추가하여 해당 `state` 변화를 감지합니다.
...
> **Custom Hook**을 사용할 때마다 그 안의 `state`와 `effect`는 완전히 독립적 입니다.
